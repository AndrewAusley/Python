...
class ArrayWalk:
    ...
    def calcVerts(self):
        for col in range(0, len(array[0][:])):
            for row in range(0, len(array[:][0]) - 3):
                prod = array[row][col] * array[row + 1][col] * array[row + 2][col] * array[row + 3][col]
                if prod > self.maxprod:
                    print array[row][col], array[row + 1][col], array[row + 2][col], array[row + 3][col]
                    self.maxprod = prod
    ...
    def calcHorz(self):
        for row in range(0, len(array[0][:])):
            for col in range(0, len(array[:][0]) - 3):
                ...
                if prod > self.maxprod:
                    print array[row][col], array[row][col + 1], array[row][col + 2], array[row][col + 3]
                    self.maxprod = prod

    def calcRightDiag(self):
        for col in range(0, len(array[0][:]) - 3):
            for row in range(0, len(array[:][0]) - 3):
                prod = array[row][col] * array[row + 1][col + 1] * array[row + 2][col + 2] * array[row + 3][col + 3]
                if prod > self.maxprod:
                    print array[row][col], array[row + 1][col + 1], array[row + 2][col + 2], array[row + 3][col + 3]
                    self.maxprod = prod

    def calcLeftDiag(self):
